...
class Generator(torch.nn.Module):
    ...
    def __init__(self):
        super().__init__()
        input_layer = []
        input_layer.append(torch.nn.Linear(13, latent_dim//4))
        input_layer.append(torch.nn.ReLU())
        input_layer.append(torch.nn.Linear(latent_dim//4, latent_dim//2))
        input_layer.append(torch.nn.ReLU())
        input_layer.append(torch.nn.Linear(latent_dim//2, latent_dim))
        input_layer.append(torch.nn.ReLU())

        self.transconv0 = GeneraterBlock(latent_dim*2, 256, (4, 1, 1), (4, 1, 1))
    ...
    def forward(self, x, input):
        ...
        input_output = torch.reshape(input_output, (-1, latent_dim, 1, 1, 1))
...
if __name__ == "__main__":
    ...
    generator = Generator()
    x = generator(latent_vector, input)
    print(x.shape)